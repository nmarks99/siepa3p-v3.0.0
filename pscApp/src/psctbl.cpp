...
#define ALARM_RET(PREC, ALARM_TYPE, ALARM_SEV) \
    {                                                           \
        int junk = recGblSetSevr(PREC, ALARM_TYPE, ALARM_SEV);  \
        junk += 1;                                              \
        return 0;                                               \
    }
...
//    timeprintf("x0=%d, x1=%d, y0=%f, y1=%f, r0=%f, r1=%f\n", x0, x1, y0, y1, r0, r1);
    ...
    double a_data[] = {    pow(x0,5),    pow(x0,4),   pow(x0,3), pow(x0,2), x0, 1,
    ...
    try {
        ...
        if(send_tg)
        {
            ...
            if(send_tr)
            {
                ...
                priv->psc->queueSend(priv->bid, tmp.data(), tmp.size());
                priv->psc->flushSend();
